...
def build_story() -> list:
    ...
    cover_left = [
        ...
        para("AI와 함께 만들고, 플레이로 완성한 기록", "CoverSub"),
        para(
            "바이브코딩을 결과물의 변명이 아니라, 게임 프로그래머의 판단을 더 빠르게 구현·검증·수정하기 위한 개발 방식으로 사용한 v0.2 사례 보고서.",
            "BodyKRStyle",
        ),
        Spacer(1, 9),
        panel(
    ...
    story += [
        ...
        para(
            "또한 빠른 반복이 구조적 부채로 바뀌지 않도록 규칙 모듈, 테스트, Git 커밋, HANDOFF, 실제 공개 번들 확인을 함께 관리했다. 현재 v0.2는 일반·젠가 싱글 모드, 로비, DB 랭킹, 게스트 우선 프로필과 Google 계정 연동을 공개 빌드에 포함한다. 다음 단계의 프라이빗 룸과 최대 20인 공개 멀티를 고려해 판정 책임과 상태 경계를 계속 분리하고 있다.",
        ),
        Spacer(1, 4),
        panel(
            [
    ...
    image_block = captioned_image(
        ...
        "v0.2 게임 화면. 블록 배치, 실시간 점수, 채팅, 해설, 고도 UI와 모바일 대응 조작을 한 화면에 통합했다.",
        CONTENT_W,
        78 * mm,
    )
    story += image_block
    ...
    story += [
        para(
            "현재 공개판 v0.2는 <b>일반·젠가 싱글 모드에서 물리와 조작을 검증하는 빌드</b>다. 블록은 차례 시작 시 공개되고, 60초 동안 위치와 Y축 회전, 가로/세로 자세를 조정해 놓는다. 중앙 정렬과 안정성에 따라 점수를 얻고, 경기장을 이탈한 블록은 소유자 점수에서 차감된다. 젠가 모드는 첫 이벤트가 비어 보이지 않도록 초기 지지 블록을 갖춘 상태에서 시작하며, 특수 문어 블록은 아래 블록을 붙잡아 탑의 상황을 바꾼다.",
        ),
        para(
    ...
    story += [
        ...
        para(
            "<b>모델과 추론 강도도 작업 조건으로 명시한다.</b> 물리 상태 경쟁, 구조 리팩터링, 출시 판정처럼 실패 비용이 큰 일은 GPT-5.6 Terra / High 수준의 깊은 추론을 사용하고, 단순 문구나 기계적 수정은 더 가벼운 작업으로 분리한다. 모델을 숨은 변수로 두지 않아 작업자가 기대하는 정밀도와 비용을 맞춘다.",
        ),
        Spacer(1, 5),
    ]
    model_cards = Table(
    ...
    architecture = Table(
        [
            ...
            [
                card("UI·계정·언어", "Lobby · PlayerProfile · i18n · KO/EN/JP", colors.HexColor("#F2EBFF"), 52 * mm),
                card("결과 연출", "demolitionArrow · 점수 · 해설 · 오디오", SOFT_YELLOW, 52 * mm),
                card("서버·검증·출시", "D1 ranking/profile · tests · Cloudflare · Git/HANDOFF", colors.HexColor("#F4F4F4"), 52 * mm),
            ],
        ],
        colWidths=[56 * mm, 56 * mm, 56 * mm],
    )
    ...
    story += [
        ...
        para(
            "하지만 <b>TowerGame.tsx는 현재도 약 2,716줄</b>로 크다. 물리 루프의 연결점과 UI 상태가 아직 한 컴포넌트에 남아 있어, 이 문서에서는 모듈화가 끝났다고 과장하지 않는다. 다음 단계는 월드 세션, 턴 상태 머신, 점수/결산, 카메라 컨트롤러를 분리하고, 서버 권한 이벤트와 클라이언트 표현을 다른 계층으로 나누는 것이다.",
        ),
    ]
    quality_data = [
        ["검증 계층", "현재 기준", "막는 회귀"],
        ["규칙·물리 단위 테스트", "53개", "배치 범위, 점수, 젠가, 문어, 화살, 낙하 보조, 계정/랭킹"],
    ...
    release_flow = [
        ...
        ["3. Behavior", "규칙·물리 테스트 53개, 제품 요구 테스트 2개", "새 기능으로 넘어가지 않고 회귀 원인 해결"],
    ...
    story += [
        para(
            "프로토타입 단계에서는 빠르게 실험했지만, 난이도와 물리 규칙이 기준점에 도달한 뒤 버전을 <b>0.1.0</b>으로 고정했고, 로비·랭킹·프로필·재화 UI와 공개 환경 분리를 거쳐 현재 <b>0.2.0</b>으로 확장했다. 이후 커밋은 특수 블록, 난이도, 모바일 UI, 젠가, 화살 연출처럼 독립적인 목적을 갖는다. 잘못된 카메라 추적이나 과도한 안정 보조는 새 구현 위에 보정하지 않고 롤백해, 결정의 역사를 Git에서 추적할 수 있게 했다.",
        ),
        para(
    ...
    release_cards = Table(
        [[
            metric("a654f88", "현재 게임 HEAD", SOFT_BLUE, 49 * mm),
            metric("0.2.0", "패키지 버전", SOFT_MINT, 49 * mm),
            metric("55", "자동 검증 항목", SOFT_YELLOW, 49 * mm),
        ]],
    ...
    roadmap = [
        ...
        ["계정", "게스트 우선 + Google 연동 + D1 프로필", "게스트 토큰, 중복 닉네임, 재접속, 서버 권한 보존"],
    ...
    story += [
        ...
        para(
            "v0.2에서 규칙과 연출을 분리하고 랭킹·프로필을 D1에 연결한 이유가 여기서 드러난다. gameplayRules의 판정은 서버 공용 규칙으로 옮길 수 있고, demolitionArrow와 표정 변화는 확정 이벤트를 소비하는 클라이언트 표현으로 남길 수 있다. 다음 버전은 기능 추가보다 이 권한 경계를 먼저 검증한 뒤 프라이빗 룸, 공개 멀티 순으로 확장할 계획이다.",
        ),
        panel(
            [
    ...
    story += page_heading(
        "11",
        "정리",
        "AI를 쓰는 방식은 ‘빠른 생성’보다 ‘빠른 검증’에 가깝다",
    ...
    principles = [
        ...
        ["완료를 공개판으로 정의한다", "테스트와 빌드 뒤 GitHub·Cloudflare 실제 결과를 확인한다", "v0.2, 55개 자동 검증, 배포 이력"],
    ]
    ...
    story += [
        ...
        para(
            "AI는 반복 비용을 낮춰 더 많은 플레이 테스트와 더 빠른 실험을 가능하게 한다. 게임 프로그래머는 그 여유를 더 많은 기능에만 쓰는 것이 아니라, 재미의 책임 경계를 분명히 하고 실패를 재현하며 다음 버전이 감당할 구조를 만드는 데 써야 한다. 우당탕 타워 v0.2는 그 작업 방식을 공개 빌드, 계정, 랭킹까지 확장한 현재 결과다.",
        ),
        Spacer(1, 8),
        panel(
            [
                para("PORTFOLIO TAKEAWAY", "Kicker"),
                para("AI에게 정답을 받은 프로젝트가 아니라, AI가 빠르게 움직여도 게임의 방향과 품질을 흔들지 않게 만든 프로젝트.", "Quote"),
                Spacer(1, 4),
        ...
        Spacer(1, 13),
        para("우당탕 타워 v0.2 · 박찬욱 · Game Client Programmer · 2026.08.04", "Caption"),
    ]
